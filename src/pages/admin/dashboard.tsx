  ...
  const chartData = interval.map(point => {
    ...
    const filtered = orders.filter(o => {
      const localDate = new Date(
        new Date(o.createdAt).toLocaleString('en-US', { timeZone: 'Asia/Bangkok' })
      )
      return view === 'year'
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white p-4 rounded border border-[#e5d5cb] shadow">
            ...
            <p className="text-2xl font-bold text-green-700">
              ฿{chartData.reduce((a, c) => a + c.revenue, 0).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded border border-[#e5d5cb] shadow">
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={chartData}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="orders" stroke="#9c191d" strokeWidth={2} name="Orders" />
              <Line type="monotone" dataKey="revenue" stroke="#8cbf5f" strokeWidth={2} name="Revenue (฿)" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
